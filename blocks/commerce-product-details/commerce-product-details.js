...
/**
 * Commerce Product Details Block
 *
 * Ported from the Adobe Commerce EDS boilerplate (hlxsites/aem-boilerplate-commerce)
 * `blocks/product-details`, renamed to `commerce-product-details` throughout
 * (folder, CSS classes, block name) so it can coexist with this project's
 * existing hand-built `product-details` block used on the other 14 product
 * pages. Only pages that explicitly use THIS block are wired to Adobe
 * Commerce via GraphQL; every other product page is untouched.
 *
 * Authoring model: the block itself takes no authored rows/cells — all
 * content comes live from the Commerce GraphQL endpoint via the SKU
 * resolved by scripts/commerce.js getProductSku() (page metadata `sku`,
 * or a `defaultsku` block config value, or the URL path).
 */
...
export default async function decorate(block) {
  // Design tokens the drop-in components' self-injected CSS expects
  // (--spacing-*, --color-neutral-*, --shape-*, --type-*). Scoped to this
  // block's usage only — not loaded site-wide.
  loadCSS(`${window.hlx.codeBasePath}/styles/commerce-tokens.css`);
  ...
  const fragment = document.createRange().createContextualFragment(`
    <div class="commerce-product-details__alert"></div>
    <div class="commerce-product-details__wrapper">
      <div class="commerce-product-details__left-column">
        <div class="commerce-product-details__gallery"></div>
      </div>
      <div class="commerce-product-details__right-column">
        <div class="commerce-product-details__header"></div>
        <div class="commerce-product-details__price"></div>
        <div class="commerce-product-details__gallery"></div>
        <div class="commerce-product-details__short-description"></div>
        <div class="commerce-product-details__gift-card-options"></div>
        <div class="commerce-product-details__configuration">
          <div class="commerce-product-details__options"></div>
          <div class="commerce-product-details__quantity"></div>
          <div class="commerce-product-details__buttons">
            <div class="commerce-product-details__buttons__add-to-cart"></div>
            <div class="commerce-product-details__buttons__add-to-wishlist"></div>
          </div>
        </div>
        <div class="commerce-product-details__description"></div>
        <div class="commerce-product-details__attributes"></div>
      </div>
    </div>
  `);

  const $alert = fragment.querySelector('.commerce-product-details__alert');
  const $gallery = fragment.querySelector('.commerce-product-details__gallery');
  const $header = fragment.querySelector('.commerce-product-details__header');
  const $price = fragment.querySelector('.commerce-product-details__price');
  const $galleryMobile = fragment.querySelector('.commerce-product-details__right-column .commerce-product-details__gallery');
  const $shortDescription = fragment.querySelector('.commerce-product-details__short-description');
  const $options = fragment.querySelector('.commerce-product-details__options');
  const $quantity = fragment.querySelector('.commerce-product-details__quantity');
  const $giftCardOptions = fragment.querySelector('.commerce-product-details__gift-card-options');
  const $addToCart = fragment.querySelector('.commerce-product-details__buttons__add-to-cart');
  const $wishlistToggleBtn = fragment.querySelector('.commerce-product-details__buttons__add-to-wishlist');
  const $description = fragment.querySelector('.commerce-product-details__description');
  const $attributes = fragment.querySelector('.commerce-product-details__attributes');
